...
function FoodCheckoutContent() {
  ...
  const estimatedPrepTime = useMemo(
    () => Math.max(...items.map((i) => i.preparationTime ?? 0), 0),
    [items],
  );

  // ── Form validation ──────────────────────────────────────────────
  const validateForm = useCallback((): boolean => {
    if (deliveryType === "delivery" && !foodAddress) {
      setError(t("noDeliveryAddressSet"));
      return false;
    }
    setError(null);
    return true;
  }, [deliveryType, foodAddress, t]);

  const isFormValid = useMemo(() => {
    if (items.length === 0) return false;
    if (deliveryType === "delivery") return !!foodAddress;
    return true;
  }, [items, deliveryType, foodAddress]);

  // ── Build delivery address payload ───────────────────────────────
  const deliveryAddressPayload = useMemo(() => {
    if (deliveryType !== "delivery" || !foodAddress) return null;
    return {
      addressLine1: foodAddress.addressLine1,
      addressLine2: foodAddress.addressLine2 || "",
      city: foodAddress.city,
      mainRegion: foodAddress.mainRegion,
      phoneNumber: foodAddress.phoneNumber || "",
      location: foodAddress.location || null,
    };
  }, [deliveryType, foodAddress]);
  ...
  const handleCardPayment = useCallback(async () => {
    ...
    try {
      ...
      const result = await initPayment({
        ...
        customerPhone: foodAddress?.phoneNumber || "",
        orderNumber,
      });
...
function FoodCheckoutSkeleton({ isDarkMode }: { isDarkMode: boolean }) {
  ...
  const cardBorder = isDarkMode ? "border-gray-700/40" : "border-gray-200";
