...
/**
 * Pre-fetched data passed from the server component (`page.tsx`) so the page
 * paints with real content on the first frame instead of running the
 * shop-doc → products → collections → reviews waterfall in the browser.
 *
 * The seed is consumed only when `matchKey` matches the URL's shopId — a
 * stale prefetch or mid-flight navigation falls back to the client fetch.
 */
export interface ShopBundleInitialData {
  shop: ShopData;
  products: Product[];
  ...
  hasMore: boolean;
  /** The shopId that produced this data — guard against URL mismatch. */
  matchKey: string;
}

interface ShopDetailPageProps {
  initialData?: ShopBundleInitialData | null;
}
...
export default function ShopDetailPage({
  ...
  // ── SSR seed gate ──────────────────────────────────────────────────────────
  // Seed is only valid when it lines up with the URL we're rendering. A stale
  // prefetch or mid-flight nav falls back to the normal client fetch.
  const seedValid = initialData != null && initialData.matchKey === shopId;
  const seedRef = useRef<ShopBundleInitialData | null>(
    seedValid ? initialData : null,
  );
  // One-shot guard: when seeded, skip the on-mount auto-fetches in each
  // useEffect below. The consumer-effect at the bottom flips this to false
  // after the first render so subsequent filter/sort/search changes still
  // trigger a fresh fetch as expected.
  const seedActiveRef = useRef<boolean>(seedValid);

  // ── Shop data ──────────────────────────────────────────────────────────────
  const [shopData, setShopData] = useState<ShopData | null>(
    () => seedRef.current?.shop ?? null,
  );
  const [shopCollections, setShopCollections] = useState<ShopCollection[]>(
    () => seedRef.current?.collections ?? [],
  );
  const [reviews, setReviews] = useState<Review[]>(
    () => seedRef.current?.reviews ?? [],
  );
  const [isLoading, setIsLoading] = useState(() => !seedRef.current);
  const [error, setError] = useState<string | null>(null);

  // ── Product state ──────────────────────────────────────────────────────────
  // Mirrors Flutter's allProductsNotifier / dealProductsNotifier / bestSellersNotifier
  const initialDerived = seedRef.current
    ? deriveProductArrays(seedRef.current.products)
    : null;
  const [allProducts, setAllProducts] = useState<Product[]>(
    () => initialDerived?.all ?? [],
  );
  const [dealProducts, setDealProducts] = useState<Product[]>(
    () => initialDerived?.deals ?? [],
  );
  const [bestSellers, setBestSellers] = useState<Product[]>(
    () => initialDerived?.bestSellers ?? [],
  );

  const [isProductsLoading, setIsProductsLoading] = useState(false);
  const [isLoadingMore, setIsLoadingMore] = useState(false);
  const [isInitialProductLoad, setIsInitialProductLoad] = useState(
    () => !seedRef.current,
  );
  const [hasMore, setHasMore] = useState(
    () => seedRef.current?.hasMore ?? true,
  );
  ...
  const [specFacets, setSpecFacets] = useState<SpecFacets>(
    () => seedRef.current?.specFacets ?? {},
  );

  // ── UI state ───────────────────────────────────────────────────────────────
  const [isDarkMode, setIsDarkMode] = useState(false);
  const [isMobile, setIsMobile] = useState(false);
  // Mirror fetchShopData()'s default-tab logic when seeded — without this the
  // initial tab is "allProducts" even for shops with home banners.
  const [activeTab, setActiveTab] = useState<TabType>(() => {
    const seededShop = seedRef.current?.shop;
    if (
      seededShop?.homeImageUrls &&
      seededShop.homeImageUrls.length > 0
    ) {
      return "home";
    }
    return "allProducts";
  ...
  // Skip fetchShopData on first render when the seed already populated it.
  useEffect(() => {
   ...
   *
   * When seeded, the very first run is a no-op: products/collections/reviews
   * are already in state from the seed. The seed is consumed (cleared) so
   * a subsequent shop-id change still triggers a full reset + refetch.
   */
  useEffect(() => {
    if (!shopData) return;

    if (seedRef.current && seedRef.current.shop.id === shopData.id) {
      // Consume the seed: future shopId changes should fall through to the
   ...
   *
   * On the very first render after a seed, all deps still hold their initial
   * values — the effect would normally fire and re-fetch the same first page
   * the server already returned. The `seedActiveRef` guard suppresses that
   * one-time redundant fetch, then flips off so real filter/sort/search
