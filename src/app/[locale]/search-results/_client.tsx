...
/**
 * Server → client data shape. Shops use the raw `{seconds, nanoseconds}` form
 * for `createdAt` because the firebase `Timestamp` class isn't serializable
 * across the RSC boundary; the client reconstructs Timestamps when seeding.
 */
export interface SearchResultsInitialData {
  ...
  hasMore: boolean;
  /** The query string the SSR fetched for. Mismatch → ignore the seed. */
  matchKey: string;
}
...
export default function SearchResultsPage({
  ...
  // ── SSR seed ───────────────────────────────────────────────────────────────
  // Only honour the seed if the visible query matches what we pre-fetched.
  const seededRef = useRef<boolean>(
    initialData != null &&
      initialData.matchKey === query &&
      initialData.products.length > 0,
  );
  const seedRef = useRef<SearchResultsInitialData | null>(
  ...
  const fetchDoneRef = useRef<boolean>(seededRef.current);
  ...
  const [shops, setShops] = useState<Shop[]>(() => {
    const raw = seedRef.current?.shopsRaw;
    if (!raw) return [];
    return raw.map((s) => ({
      ...s,
  ...
  // When SSR-seeded for this exact query, we bail here — the seed effect
  ...
  // Also responsible for *consuming* the SSR seed — both effects fire in
  // declaration order on mount, so the query effect above has already
  // short-circuited by the time this runs.
  useEffect(() => {
    if (!query) return;
