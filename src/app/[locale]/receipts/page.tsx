...
export default function ReceiptsPage() {
  ...
  // ── Market receipts state ────────────────────────────────────────
  const [marketReceipts, setMarketReceipts] = useState<MarketReceipt[]>([]);
  const [isMarketLoading, setIsMarketLoading] = useState(false);
  const [isMarketInitialLoad, setIsMarketInitialLoad] = useState(true);
  const [hasMarketMore, setHasMarketMore] = useState(true);
  const [lastMarketDocument, setLastMarketDocument] =
    useState<QueryDocumentSnapshot<DocumentData> | null>(null);
  ...
  // ── Fetch market receipts ────────────────────────────────────────
  // Same Firestore path the Flutter app uses and the market receipt detail
  // page reads from: users/{uid}/marketReceipts, ordered by timestamp desc.
  const fetchMarketReceipts = useCallback(
    async (loadMore = false) => {
      if (!user || isMarketLoading) return;
      setIsMarketLoading(true);
      try {
        const ref = collection(db, "users", user.uid, "marketReceipts");
        let q = query(ref, orderBy("timestamp", "desc"), limit(RECEIPTS_LIMIT));
        if (loadMore && lastMarketDocument) {
          q = query(
            ref,
            orderBy("timestamp", "desc"),
            startAfter(lastMarketDocument),
            limit(RECEIPTS_LIMIT),
          );
        }
        const snapshot = await getDocs(q);
        if (snapshot.docs.length > 0) {
          const items: MarketReceipt[] = snapshot.docs.map((doc) => {
            const data = doc.data();
            return {
              id: doc.id,
              orderId: data.orderId || "",
          ...
          setLastMarketDocument(snapshot.docs[snapshot.docs.length - 1]);
          if (loadMore) setMarketReceipts((prev) => [...prev, ...items]);
          else setMarketReceipts(items);
          setHasMarketMore(snapshot.docs.length >= RECEIPTS_LIMIT);
        } else {
          setHasMarketMore(false);
        }
      } catch (error) {
        console.error("Error fetching market receipts:", error);
        alert(
          `Error: ${l("Receipts.errorFetchingReceipts") || "Error fetching receipts"}`,
        );
      } finally {
        setIsMarketLoading(false);
        setIsMarketInitialLoad(false);
        setMarketFetched(true);
      }
    },
    [user, isMarketLoading, lastMarketDocument],
  );
  ...
  useEffect(() => {
    ...
    if (activeTab === "market" && user && !marketFetched) {
      fetchMarketReceipts();
    }
  }, [activeTab, user, foodFetched, marketFetched]);
  ...
  const refreshMarketReceipts = useCallback(async () => {
    setMarketReceipts([]);
    setLastMarketDocument(null);
    setHasMarketMore(true);
    setIsMarketInitialLoad(true);
    setMarketFetched(false);
    await fetchMarketReceipts();
  }, [fetchMarketReceipts]);
  ...
  const loadMoreMarketReceipts = useCallback(() => {
    if (hasMarketMore && !isMarketLoading) fetchMarketReceipts(true);
  }, [hasMarketMore, isMarketLoading, fetchMarketReceipts]);
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (scrollHeight - scrollTop <= clientHeight + 200) {
        ...
        else if (activeTab === "food") loadMoreFoodReceipts();
        else if (activeTab === "market") loadMoreMarketReceipts();
      }
    };
    ...
    activeTab,
    loadMoreReceipts,
    loadMoreFoodReceipts,
    loadMoreMarketReceipts,
  ]);
  ...
  const goToMarketReceiptDetail = (receipt: MarketReceipt) => {
    // Matches the route we built in the market receipt detail page.
    router.push(`/${locale}/market-receipts/${receipt.id}`);
  };
    ...
    >
      ...
      >
        <div className="max-w-4xl mx-auto px-3 sm:px-6">
          ...
          {/* Tab switcher */}
          {/* Horizontal scroll on narrow viewports so three tabs don't wrap. */}
          <div className="flex gap-1 pb-3 overflow-x-auto scrollbar-none -mx-1 px-1">
            <button
              onClick={() => setActiveTab("product")}
              className={`flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-semibold transition-all whitespace-nowrap flex-shrink-0 ${
                activeTab === "product"
            ...
            >
              ...
              {l("Receipts.marketOrders") || "Market"}
            </button>
          </div>
        </div>
      </div>
        ...
        isMarketInitialLoad ? (
          ...
          <div className="text-center py-16">
            ...
            <p
              className={`text-xs max-w-xs mx-auto ${isDarkMode ? "text-gray-400" : "text-gray-500"}`}
            >
              {l("Receipts.marketReceiptsWillAppearHere") ||
                "Your market order receipts will appear here."}
            </p>
          </div>
        ) : (
          /* Market Receipts List */
          <div className="space-y-3">
            {marketReceipts.map((receipt) => (
              <div
                key={receipt.id}
                onClick={() => goToMarketReceiptDetail(receipt)}
                className={`rounded-2xl border overflow-hidden cursor-pointer hover:shadow-md hover:-translate-y-0.5 transition-all ${
                  isDarkMode
                    ? "bg-gray-800 border-gray-700"
                    : "bg-white border-gray-100"
              ...
              >
                {/* Main row */}
                <div className="px-4 py-3 flex items-center gap-3">
                  {/* Icon — emerald palette to signal Market, not food/product */}
                  <div
                    className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${
                  ...
                  <div className="flex-1 min-w-0">
                    <h3
                      className={`text-sm font-semibold truncate ${isDarkMode ? "text-white" : "text-gray-900"}`}
                    >
                      {l("Receipts.marketBrand") || "Nar24 Market"}
                    </h3>
                    <div className="flex items-center gap-3 mt-0.5">
                      <div className="flex items-center gap-1">
                        <Calendar
                ...
                {/* Paid / Pay-at-door badge strip — mirrors food layout exactly */}
                <div
                  className={`px-4 py-1.5 flex items-center justify-between border-t ${
                    isDarkMode ? "border-gray-700/60" : "border-gray-50"
                  } ${
            ...
            {/* Load More Spinner */}
            {isMarketLoading && !isMarketInitialLoad && (
              <div className="flex justify-center py-8">
            ...
            {/* End of List */}
            {!hasMarketMore && marketReceipts.length > 0 && (
              <p
                className={`text-center text-[11px] py-4 ${isDarkMode ? "text-gray-600" : "text-gray-400"}`}
              >
                {l("Receipts.noMoreReceipts") || "No more receipts to load"}
