...
export interface DynamicMarket2InitialData {
  products: Product[];
...
interface DynamicMarketPageProps {
  initialData?: DynamicMarket2InitialData | null;
}

const DynamicMarketPage: React.FC<DynamicMarketPageProps> = ({
  initialData,
} = {}) => {
...
const seqRef = useRef(0);
  ...
  // ── SSR seed ───────────────────────────────────────────────────────────────
  // Only honour the seed if the URL the client sees matches the URL the server
  // pre-fetched for. Different match → ignore seed, fall through to normal fetch.
  ...
  const seedRef = useRef<DynamicMarket2InitialData | null>(
...
useEffect(() => {
  ...
  // Fast path: SSR-seeded for this exact URL with no filters yet — keep state.
  if (seededRef.current) {
    seededRef.current = false; // one-shot
    return;
  }
