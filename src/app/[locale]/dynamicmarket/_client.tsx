...
export interface DynamicMarketInitialData {
  products: Product[];
  specFacets?: SpecFacets;
  hasMore: boolean;
  /** URL param triplet that produced this data — used to detect SSR/URL mismatch. */
  matchKey: string;
}

interface DynamicMarketPageProps {
  initialData?: DynamicMarketInitialData | null;
}
...
export default function DynamicMarketPage({
  ...
  // ── SSR seed ───────────────────────────────────────────────────────────────
  // The seed only applies when the client URL matches what the server fetched.
  // Different match → ignore the seed and let the normal fetch run.
  const currentMatchKey = `${category}|${subcategory}|${subsubcategory}`;
  ...
  useEffect(() => {
    ...
    // Fast path: SSR-seeded with matching URL and no filters yet — keep state.
