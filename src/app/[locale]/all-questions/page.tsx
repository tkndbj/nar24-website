...
const AllQuestionsPage: React.FC<AllQuestionsPageProps> = ({}) => {
  ...
  const LoadingSkeleton = () => (
    <div className="space-y-4 p-4">
      {[1, 2, 3].map((i) => (
        <div
          key={i}
          className={`${colors.containerBg} rounded-xl p-6 space-y-4 animate-pulse`}
        >
  ...
  const QuestionCard = ({ question }: { question: Question }) => {
    ...
    const answererName =
      question.answererName || sellerInfo?.name || t("seller");
    const answererImage =
          ...
          <div className="flex items-center justify-between max-w-6xl mx-auto">
            ...
            {sellerInfo && (
              <div className="flex items-center gap-2">
                ...
                <span
                  className={`text-sm font-medium ${colors.text} hidden sm:block`}
                >
                  {sellerInfo.name}
                </span>
              </div>
      ...
      <div className="max-w-4xl mx-auto px-2 md:px-0" ref={scrollContainerRef}>
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-12 md:py-20 px-4">
            <div className="w-20 h-20 md:w-24 md:h-24 rounded-full bg-gray-100 dark:bg-surface flex items-center justify-center mb-3 md:mb-4">
