  ...
  return (
    ...
    >
      <div className="flex max-w-7xl mx-auto">
            ...
            <div className="p-3">
              ...
              <div className="space-y-4">
                ...
                {getAvailableSubSubcategories().length > 0 && (
                  <div>
                    ...
                    >
                      ...
                      {expandedSections.subcategory ? <ChevronUp size={14} className="text-gray-400" /> : <ChevronDown size={14} className="text-gray-400" />}
                    </button>
                    {expandedSections.subcategory && (
                      <div className="mt-1.5 space-y-1.5 max-h-40 overflow-y-auto">
                        {getAvailableSubSubcategories().map((subSub) => (
                          <label key={subSub} className="flex items-center space-x-2 cursor-pointer group">
                            <input type="checkbox" checked={filters.subcategories.includes(subSub)} onChange={() => handleToggleFilter("subcategories", subSub)} className="w-3 h-3 rounded border-gray-300 dark:border-gray-600 text-orange-500 focus:ring-orange-500 focus:ring-1" />
                            <span className={`text-xs group-hover:text-orange-500 transition-colors ${isDarkMode ? "text-gray-300" : "text-gray-600"}`}>{getLocalizedSubcategoryName(subSub)}</span>
                          </label>
                        ))}
                      </div>
                    )}
                ...
                {availableColors.length > 0 && (
                  <div>
                    <button onClick={() => setExpandedSections((prev) => ({ ...prev, color: !prev.color }))} className="w-full flex items-center justify-between text-left py-1.5">
                      <span className={`font-medium text-xs ${isDarkMode ? "text-white" : "text-gray-900"}`}>{t("DynamicMarket.colors")}</span>
                      {expandedSections.color ? <ChevronUp size={14} className="text-gray-400" /> : <ChevronDown size={14} className="text-gray-400" />}
                    </button>
                    {expandedSections.color && (
                      <div className="mt-1.5 space-y-1.5 max-h-40 overflow-y-auto">
                        {availableColors.map((colorData) => (
                          <label key={colorData.name} className="flex items-center space-x-2 cursor-pointer group">
                            <input type="checkbox" checked={filters.colors.includes(colorData.name)} onChange={() => handleToggleFilter("colors", colorData.name)} className="w-3 h-3 rounded border-gray-300 dark:border-gray-600 text-orange-500 focus:ring-orange-500 focus:ring-1" />
                            <div className="w-3 h-3 rounded border border-gray-300 flex-shrink-0" style={{ backgroundColor: colorData.color }} />
                            <span className={`text-xs group-hover:text-orange-500 transition-colors ${isDarkMode ? "text-gray-300" : "text-gray-600"}`}>{getLocalizedColorName(colorData.name)}</span>
                          </label>
                        ))}
                      </div>
                    )}
                ...
                {globalBrands.length > 0 && (
                  <div>
                    <button onClick={() => setExpandedSections((prev) => ({ ...prev, brand: !prev.brand }))} className="w-full flex items-center justify-between text-left py-1.5">
                      <span className={`font-medium text-xs ${isDarkMode ? "text-white" : "text-gray-900"}`}>{t("DynamicMarket.brands")}</span>
                    ...
                    {expandedSections.brand && (
                      <div className="mt-1.5 space-y-1.5">
                        <div className="relative mb-2">
                          ...
                          <input type="text" placeholder={t("DynamicMarket.searchBrands")} value={brandSearch} onChange={(e) => setBrandSearch(e.target.value)} className={`w-full pl-7 pr-2 py-1.5 text-xs rounded border ${isDarkMode ? "bg-gray-700 border-gray-600 text-white placeholder-gray-400" : "bg-white border-gray-200 text-gray-900 placeholder-gray-400"} focus:ring-1 focus:ring-orange-500 focus:border-orange-500`} />
                        </div>
                        <div className="max-h-40 overflow-y-auto space-y-1.5">
                          {filteredBrands.map((brand) => (
                            <label key={brand} className="flex items-center space-x-2 cursor-pointer group">
                              <input type="checkbox" checked={filters.brands.includes(brand)} onChange={() => handleToggleFilter("brands", brand)} className="w-3 h-3 rounded border-gray-300 dark:border-gray-600 text-orange-500 focus:ring-orange-500 focus:ring-1" />
                              <span className={`text-xs group-hover:text-orange-500 transition-colors ${isDarkMode ? "text-gray-300" : "text-gray-600"}`}>{brand}</span>
                            </label>
                          ))}
                        </div>
                      </div>
                ...
                {/* Price Range */}
                <div>
                  <button onClick={() => setExpandedSections((prev) => ({ ...prev, price: !prev.price }))} className="w-full flex items-center justify-between text-left py-1.5">
                    <span className={`font-medium text-xs ${isDarkMode ? "text-white" : "text-gray-900"}`}>{t("DynamicMarket.priceRange")}</span>
                  ...
                  {expandedSections.price && (
                    <div className="mt-1.5 space-y-2">
                      <div className="flex items-center space-x-2">
                        <input type="number" placeholder={t("DynamicMarket.min")} value={minPriceInput} onChange={(e) => setMinPriceInput(e.target.value)} className={`w-full px-2 py-1.5 text-xs rounded border ${isDarkMode ? "bg-gray-700 border-gray-600 text-white" : "bg-white border-gray-200 text-gray-900"} focus:ring-1 focus:ring-orange-500 focus:border-orange-500`} />
                        <span className={`text-xs ${isDarkMode ? "text-gray-400" : "text-gray-500"}`}>-</span>
                        <input type="number" placeholder={t("DynamicMarket.max")} value={maxPriceInput} onChange={(e) => setMaxPriceInput(e.target.value)} className={`w-full px-2 py-1.5 text-xs rounded border ${isDarkMode ? "bg-gray-700 border-gray-600 text-white" : "bg-white border-gray-200 text-gray-900"} focus:ring-1 focus:ring-orange-500 focus:border-orange-500`} />
                      </div>
                      <button onClick={handleSetPriceFilter} className="w-full py-1.5 text-xs bg-orange-500 text-white rounded hover:bg-orange-600 transition-colors">{t("DynamicMarket.apply")}</button>
                    </div>
                  )}
                </div>
              </div>
        ...
        <div
          className={`
            hidden lg:block sticky top-0 h-screen w-64 z-40
            ${isDarkMode ? "bg-gray-800" : "bg-white"}
            border-r ${isDarkMode ? "border-gray-700" : "border-gray-200"}
            overflow-y-auto overflow-x-hidden flex-shrink-0 pb-20
          `}
