...
export interface DynamicTerasInitialData {
  products: Product[];
  specFacets?: SpecFacets;
  hasMore: boolean;
  /** URL params signature that produced this data — guards against URL drift. */
  matchKey: string;
}

interface DynamicTerasPageProps {
  initialData?: DynamicTerasInitialData | null;
}

const DynamicTerasPage: React.FC<DynamicTerasPageProps> = ({
  initialData,
} = {}) => {
  ...
  const currentMatchKey =
    `${urlParams.category}|${urlParams.selectedSubcategory}|` +
    `${urlParams.selectedSubSubcategory}|${urlParams.buyerCategory}|` +
    `${urlParams.buyerSubcategory}`;
  const seededRef = useRef<boolean>(
    initialData != null &&
      initialData.matchKey === currentMatchKey &&
      initialData.products.length > 0,
  );
  const seedRef = useRef<DynamicTerasInitialData | null>(
    seededRef.current ? initialData ?? null : null,
  );

  // ── Product state ──────────────────────────────────────────────────────────
  const [products, setProducts] = useState<Product[]>(
    () => seedRef.current?.products ?? [],
  );
  ...
  const [hasMore, setHasMore] = useState(() => seedRef.current?.hasMore ?? true);
  ...
  const fetchProducts = useCallback(
    async (page: number = 0, reset = false) => {
      ...
      try {
        ...
        const res = await fetch(`/api/fetchDynamicTerasProducts?${qp}`, {
        ...
        console.error("DynamicTerasPage fetch error:", err);
        setHasMore(false);
        if (reset) {
          setProducts([]);
        }
  ...
  // Initial load — always fetch, even without URL parameters.
  // When SSR-seeded, the seed is consumed by the second effect below; this one
  // just bails so we don't fire two simultaneous fetches on mount.
  useEffect(() => {
  ...
  // Re-fetch on sort / filter change. Also responsible for *consuming* the
  // SSR seed — both effects fire in declaration order on mount, so consuming
  // here guarantees the URL effect above has already short-circuited.
  useEffect(() => {
...
export default DynamicTerasPage;
