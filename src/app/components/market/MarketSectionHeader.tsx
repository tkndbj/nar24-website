...
export default function MarketSectionHeader({
  ...
  const { itemCount: marketCartCount } = useMarketCart();
  ...
  const isDetailPage = useMemo(() => {
    ...
    if (stripped === "/market-categories") return false;
    return (
      stripped.startsWith("/market-categories/") ||
      stripped.startsWith("/market-cart") ||
      stripped.startsWith("/market-checkout") ||
      stripped.startsWith("/market-orders") ||
      stripped.startsWith("/market-receipts") ||
      stripped.startsWith("/market-search") ||
      stripped.startsWith("/isbankmarketpayment")
    );
  }, [pathname]);
  ...
  const handleGoToMarketCart = useCallback(() => {
  ...
  const NotificationBadge = useMemo(() => {
    ...
    return (
      <span className="absolute -top-0.5 -right-0.5 w-2.5 h-2.5 bg-emerald-500 rounded-full ring-[1.5px] ring-white dark:ring-gray-900" />
    );
  }, [user, unreadNotificationsCount]);

  const MarketCartBadge = useMemo(() => {
    if (!user || marketCartCount === 0) return null;
    return (
      <span className="absolute -top-0.5 -right-0.5 min-w-[16px] h-[16px] bg-emerald-500 rounded-full flex items-center justify-center ring-[1.5px] ring-white dark:ring-gray-900">
        <span className="text-white text-[8px] font-bold leading-none px-[3px]">
          {marketCartCount > 9 ? "9+" : marketCartCount}
        </span>
      </span>
    );
  }, [user, marketCartCount]);
  ...
  const ActionIcons = ({ iconSize = 17 }: { iconSize?: number }) => {
    ...
    return (
      <div className="flex items-center flex-shrink-0">
        ...
        <button
          onClick={handleGoToMarketCart}
          className={btnCls}
          aria-label={t("header.cart")}
        >
          <ShoppingBasket size={iconSize} />
          {MarketCartBadge}
        </button>
        {sep}
        <button
  ...
  const MarketTab = ({ compact = false }: { compact?: boolean }) => {
    if (isDetailPage) return null;
    return (
      <button
      ...
      >
        <ShoppingBasket size={compact ? 12 : 13} />
        <span>{t("MarketHeader.market")}</span>
      </button>
    );
  };
          ...
          <div className="md:hidden">
            <div className="h-12 px-2 flex items-center justify-center">
              <div className="flex items-center gap-1 w-full max-w-lg">
                {isDetailPage ? <BackNav compact /> : <Logo size="sm" />}
